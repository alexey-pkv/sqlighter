...
TEST(error_message, message_defined)
{
	...
	ASSERT_EQ(std::string::npos, sqlighter::error_message(SQLIGHTER_ERR_DB_WAS_CLOSED).find(NDS));
	ASSERT_EQ(std::string::npos, sqlighter::error_message(SQLIGHTER_ERR_DB_ALREADY_OPEN).find(NDS));
